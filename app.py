...
def get_change_logs():
    response = supabase.table('trocas_cartucho').select(
        '*, usuarios(name), equipamentos(modelo), suprimentos(modelo, categoria, tipo)').order('data_troca',
                                                                                               desc=True).execute()
    return response.data
...
def run_app():
    ...
    page = st.sidebar.radio("Selecione uma página",
                            ["Registrar Troca", "Dashboard de Análise", "Gerenciar Setores", "Gerenciar Equipamentos",
    ...
    if page == "Registrar Troca":
        ...
        if not users:
            ...
            selected_user_name = st.selectbox("1. Selecione o Setor:", options=user_map.keys(), index=None,
            ...
            if selected_user_name:
                ...
                if not equipamentos_no_setor:
                    ...
                    equipamento_map = {eq['modelo']: {'id': eq['id'], 'categoria': eq['categoria']} for eq in
                                       equipamentos_no_setor}
                    selected_equipamento_modelo = st.selectbox("2. Selecione o Equipamento:",
                                                               options=equipamento_map.keys(), index=None,
                    ...
                    if selected_equipamento_modelo:
                        ...
                        if not categoria_do_equipamento:
                            st.error(
                                f"O equipamento '{selected_equipamento_modelo}' não tem uma categoria definida. Por favor, edite-o na página 'Gerenciar Equipamentos'.")
                        else:
                            suprimentos_disponiveis = get_suprimentos(categoria=categoria_do_equipamento)

                            if not suprimentos_disponiveis:
                                st.warning(
                                    f"Nenhum suprimento da categoria '{categoria_do_equipamento}' cadastrado. Vá para 'Gerenciar Suprimentos' para adicionar.")
                            else:
                                suprimento_map = {f"{sup['modelo']} ({sup['tipo']})": sup['id'] for sup in
                                                  suprimentos_disponiveis}

                                st.markdown("---")
                                with st.form("registro_troca_form"):
                                    st.info(
                                        f"Registrando para: **{selected_user_name}** | **{selected_equipamento_modelo}**")

                                    suprimento_selecionado_modelo = st.selectbox("3. Selecione o Suprimento Trocado:",
                                                                                 options=suprimento_map.keys())
                                    change_date = st.date_input("4. Data da Troca:", datetime.now())

                                    observacao = st.text_area("5. Observações (opcional):",
                                                              placeholder="Ex: Cartucho antigo falhando, manchando a página, etc.")
        ...
        if not logs:
            ...
            if df_filtrado.empty:
                ...
                with col1:
                    ...
                    fig_bar = px.bar(user_counts, x='Setor', y='Total de Trocas', title=titulo_grafico_bar,
                                     labels={'Setor': 'Nome do Setor', 'Total de Trocas': 'Quantidade'},
                ...
                fig_line = px.line(monthly_changes.sort_values(by='AnoMês'), x='AnoMês', y='Quantidade',
                                   title=titulo_grafico_linha, markers=True,
                                   labels={'AnoMês': 'Mês/Ano', 'Quantidade': 'Nº de Trocas'})
            ...
            with col_download:
                ...
                df_export = df_filtrado[
                    ['Data', 'Setor', 'Equipamento', 'Suprimento', 'Categoria', 'Tipo', 'Observação']].copy()
            ...
            if st.session_state.deleting_log_id is not None:
                ...
                st.write(
                    f"**Data:** {log_details['Data'].strftime('%d/%m/%Y')}, **Setor:** {log_details['Setor']}, **Equipamento:** {log_details['Equipamento']}, **Suprimento:** {log_details['Suprimento']}")
                    ...
                    with col_confirm:
                        if st.form_submit_button("Sim, apagar registro", type="primary"):
                            if password == st.secrets["auth"].get("delete_password", st.secrets["auth"]["password"]):
                                try:
                                    supabase.table('trocas_cartucho').delete().eq('id',
                                                                                  st.session_state.deleting_log_id).execute()
    ...
    # --- PÁGINA: GERENCIAR SETORES ---
        ...
        if st.session_state.deleting_sector_id is not None:
            st.warning(
                f"⚠️ **ATENÇÃO:** Você está prestes a apagar o setor **'{st.session_state.deleting_sector_name}'** e todos os seus **{st.session_state.deleting_sector_logs_count}** registros. Esta ação é irreversível.")
            with st.form("confirm_delete_form"):
                ...
                with col_confirm:
                    if st.form_submit_button("Confirmar Exclusão Permanente", type="primary"):
                        if password == st.secrets["auth"].get("delete_password", st.secrets["auth"]["password"]):
                            try:
                                supabase.table('trocas_cartucho').delete().eq('usuario_id',
                                                                              st.session_state.deleting_sector_id).execute()
                                supabase.table('usuarios').delete().eq('id',
                                                                       st.session_state.deleting_sector_id).execute()
                                st.success(
                                    f"O setor '{st.session_state.deleting_sector_name}' e seus registros foram removidos com sucesso!")
                ...
                for user in users_data:
                    ...
                    with st.container(border=True):
                        if st.session_state.editing_sector_id == user_id:
                            ...
                            with col2:
                                if st.button("✔️", key=f"save_{user_id}", help="Salvar alterações"):
                                    if new_name and new_name != user_name:
                                        try:
                                            supabase.table('usuarios').update({'name': new_name}).eq('id',
                                                                                                     user_id).execute()
                            ...
                            col1, col2, col3 = st.columns([0.8, 0.1, 0.1])
                            col1.markdown(f"<p style='margin-top: 5px; font-size: 1.1em;'>{user_name}</p>",
                                          unsafe_allow_html=True)

                            if col2.button("✏️", key=f"edit_{user_id}", help=f"Editar '{user_name}'",
                                           disabled=is_editing_another):
                                st.session_state.editing_sector_id = user_id
                                st.rerun()

                            if col3.button("🗑️", key=f"delete_{user_id}", help=f"Remover '{user_name}'",
                                           disabled=is_editing_another):
                                response = supabase.table('trocas_cartucho').select('id', count='exact').eq(
                                    'usuario_id', user_id).execute()
                                if response.count > 0:
    ...
    # --- PÁGINA: GERENCIAR EQUIPAMENTOS ---
        ...
        if st.session_state.deleting_equip_id is not None:
            st.warning(
                f"⚠️ **ATENÇÃO:** Você está prestes a apagar o equipamento **'{st.session_state.deleting_equip_model}'** e todos os seus **{st.session_state.deleting_equip_logs_count}** registros de troca. Esta ação é irreversível.")
            with st.form("confirm_delete_equip_form"):
                ...
                with col_confirm:
                    if st.form_submit_button("Confirmar Exclusão Permanente", type="primary"):
                        if password == st.secrets["auth"].get("delete_password", st.secrets["auth"]["password"]):
                            try:
                                supabase.table('trocas_cartucho').delete().eq('equipamento_id',
                                                                              st.session_state.deleting_equip_id).execute()
                                supabase.table('equipamentos').delete().eq('id',
                                                                           st.session_state.deleting_equip_id).execute()
                                st.success(
                                    f"O equipamento '{st.session_state.deleting_equip_model}' e seus registros foram removidos com sucesso!")
            ...
            with st.expander("Adicionar Novo Equipamento"):
                ...
                if not users_data:
                    ...
                    with st.form("novo_equipamento_form", clear_on_submit=True):
                        ...
                        categoria_equipamento = st.selectbox("Categoria do Suprimento:",
                                                             ["Cartucho de Tinta", "Suprimento Laser"])
                            ...
                            if modelo_equipamento and setor_selecionado and categoria_equipamento:
                                ...
                                try:
                                    supabase.table('equipamentos').insert(
                                        {'modelo': modelo_equipamento, 'setor_id': setor_id,
                                         'categoria': categoria_equipamento}).execute()
                ...
                for item in equipamentos_data:
                    with st.container(border=True):
                        ...
                        if col4.button("🗑️", key=f"del_equip_{equip_id}", help="Remover este equipamento"):
                            response = supabase.table('trocas_cartucho').select('id', count='exact').eq(
                                'equipamento_id', equip_id).execute()
                            if response.count > 0:
    ...
    # --- PÁGINA: GERENCIAR SUPRIMENTOS ---
        ...
        with st.expander("Adicionar Novo Suprimento ao Catálogo"):
            ...
            if st.button("Adicionar Suprimento"):
                if modelo and categoria and tipo:
                    try:
                        ...
                        # Limpa os campos
                        for key in ['new_suprimento_modelo', 'new_suprimento_categoria', 'new_suprimento_tipo_tinta', 'new_suprimento_tipo_laser']:
...
# Removida a autenticação por senha
